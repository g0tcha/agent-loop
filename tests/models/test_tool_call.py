...
def test_tool_call_valid() -> None:
    tc = ToolCall(id="call_1", tool_name="search", tool_args={"q": "cats", "limit": 5})
    assert tc.tool_name == "search"
    assert tc.tool_args == {"q": "cats", "limit": 5}


def test_tool_call_empty_args_dict() -> None:
    tc = ToolCall(id="call_1", tool_name="noop", tool_args={})
    assert tc.tool_args == {}
...
def test_tool_call_tool_args_must_be_dict() -> None:
    with pytest.raises(ValidationError):
        ToolCall(id="call_1", tool_name="x", tool_args="not-a-dict")  # type: ignore[arg-type]
